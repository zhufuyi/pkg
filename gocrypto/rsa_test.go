...
var rsaRawData = []byte("rsa|abcdefghijklmnopqrstuvwxyz1234567890")
...
func Test_rsa(t *testing.T) {
	t.Run("rsa encrypt decrypt", func(t *testing.T) {
		want := rsaRawData
		cypherData, err := RsaEncrypt(publicKey, rsaRawData) // 加密
		if err != nil {
			t.Fatal(err)
		}
		got, err := RsaDecrypt(privateKey, cypherData) // 解密
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("rsa sign verify", func(t *testing.T) {
		signData, err := RsaSign(privateKey, rsaRawData) // 私钥签名
		if err != nil {
			t.Fatal(err)
		}
		err = RsaVerify(publicKey, rsaRawData, signData) // 公钥验签
		if err != nil {
			t.Fatal(err)
		}
...
func TestRsaHex(t *testing.T) {
	t.Run("default rsa encrypt hex", func(t *testing.T) {
		...
		cypherStr, err := RsaEncryptHex(publicKey, rsaRawData) // 加密
		if err != nil {
			t.Fatal(err)
		}
		got, err := RsaDecryptHex(privateKey, cypherStr) // 解密
		if err != nil {
			t.Fatal(err)
		}
		if got != want {
...
func TestRsaBase64(t *testing.T) {
	...
	t.Run("default rsa encrypt hex", func(t *testing.T) {
		cypherStr, err := RsaSignBase64(privateKey, rsaRawData) // 加密
		if err != nil {
			t.Fatal(err)
		}
		err = RsaVerifyBase64(publicKey, rsaRawData, cypherStr) // 解密
		if err != nil {
			t.Fatal(err)
		}
